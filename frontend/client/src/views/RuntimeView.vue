<script>
    ...
    export default {
      setup() {
        ...
        async function fetchData(movie) {
          try {
            const response = await axios.get(`http://127.0.0.1:5000/api/movies/similar_runtime?movie=${movie}`)
    ...
    <div class="flex justify-center">
      <form @submit.prevent="onSubmit">   
        <label for="movie" class="mb-2 text-sm font-medium  sr-only dark:text-white">Movie</label>
        <div class="relative">
          ...
          <input id="amount" class="block pr-20 p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-green-500 dark:focus:border-green-500" placeholder="Movie" required v-model="movie">
